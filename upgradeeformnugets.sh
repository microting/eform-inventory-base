...
if (( "$GIT_STATUS" > 0 )); then
	git pull
	cd Microting.eFormInventoryBase
  CURRENT_NUMBER_OF_COMMITS=`git log --oneline | wc -l`

	PACKAGES=('Microting.eForm' 'Microting.eFormApi.BasePn' 'Microsoft.EntityFrameworkCore.Design' 'Pomelo.EntityFrameworkCore.MySql')
	PROJECT_NAME='Microting.eFormInventoryBase.csproj'
	REPOSITORY='eform-inventory-base'
